...
from robosuite.models.objects import WheelObject
...
class WheelCIP(SingleArmEnv, CIP):
    ...
    def _load_model(self):
        ...
        if self.placement_initializer is not None:
            ...
            self.placement_initializer.add_objects(self.wheel)
        else:
            self.placement_initializer = UniformRandomSampler(
                name="ObjectSampler",
                mujoco_objects=self.wheel,
    ...
    def _setup_observables(self):
        ...
        if self.use_object_obs:
            ...
            def wheel_pos(obs_cache):
                return np.array(self.sim.data.body_xpos[self.object_body_ids["wheel"]])
            ...
            @sensor(modality=modality)
            def wheel_to_eef_pos(obs_cache):
                return (
                    obs_cache["wheel_pos"] - obs_cache[f"{pf}eef_pos"]
                    if "wheel_pos" in obs_cache and f"{pf}eef_pos" in obs_cache
                    else np.zeros(3)
                )
    ...
    def _handle_xpos(self):
        ...
        """
        return self.sim.data.site_xpos[self.wheel_handle_site_id]
