...
from robosuite.models.objects import DrawerObject
...
class DrawerCIP(SingleArmEnv, CIP):
    ...
    def _setup_observables(self):
        ...
        if self.use_object_obs:
            ...
            def drawer_pos(obs_cache):
                return np.array(self.sim.data.body_xpos[self.object_body_ids["drawer"]])
            ...
            @sensor(modality=modality)
            def drawer_to_eef_pos(obs_cache):
                return (
            ...
            sensors = [drawer_pos, handle_pos, drawer_to_eef_pos, handle_to_eef_pos, slider_qpos]
    ...
    def visualize(self, vis_settings):
        ...
        if vis_settings["grippers"]:
            self._visualize_gripper_to_target(
                gripper=self.robots[0].gripper, target=self.drawer.important_sites["handle"], target_type="site"
            )
    ...
    def _handle_xpos(self):
        ...
        """
        return self.sim.data.site_xpos[self.drawer_handle_site_id]
