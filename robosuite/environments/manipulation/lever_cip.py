...
from robosuite.models.objects import LeverObject
...
class LeverCIP(SingleArmEnv, CIP):
    """
    ...
    def _load_model(self):
        ...
        if self.placement_initializer is not None:
            ...
            self.placement_initializer.add_objects(self.lever)
        else:
    ...
    def _setup_references(self):
        ...
        self.object_body_ids["lever"] = self.sim.model.body_name2id(self.lever.lever_body)
        self.lever_handle_site_id = self.sim.model.site_name2id(self.lever.important_sites["handle"])
    ...
    def _setup_observables(self):
        ...
        if self.use_object_obs:
            ...
            def lever_pos(obs_cache):
                return np.array(self.sim.data.body_xpos[self.object_body_ids["lever"]])
            ...
            @sensor(modality=modality)
            def lever_to_eef_pos(obs_cache):
                return (
                    obs_cache["lever_pos"] - obs_cache[f"{pf}eef_pos"]
                    if "lever_pos" in obs_cache and f"{pf}eef_pos" in obs_cache
                    else np.zeros(3)
                )
            ...
            sensors = [lever_pos, handle_pos, lever_to_eef_pos, handle_to_eef_pos, hinge_qpos]
    ...
    def visualize(self, vis_settings):
        ...
        if vis_settings["grippers"]:
            self._visualize_gripper_to_target(
                gripper=self.robots[0].gripper, target=self.lever.important_sites["handle"], target_type="site"
            )
    ...
    def _handle_xpos(self):
        ...
        """
        return self.sim.data.site_xpos[self.lever_handle_site_id]
