...
class DoorSlideCIP(SingleArmEnv, CIP):
    """
    ...
    def reward(self, action=None):
        ...
        #TODO: Gonna have to be adjusted based on current task
    ...
    def _load_model(self):
        ...
        self.slide = SlideObject(
            name="Slide",
            )

        self.door = DoorObject(name="Door")

        # Create placement initializer
        if self.placement_initializer is not None:
            self.placement_initializer.reset()
            self.placement_initializer.add_objects_to_sampler("SlideSampler", self.slide)
            self.placement_initializer.add_objects_to_sampler("DoorSampler", self.door)
        else:
            ...
            self.placement_initializer.append_sampler(UniformRandomSampler(
                ...
                x_range=[.1,.1], #[0.07, 0.09],
                y_range=[.9,.9], #[-0.01, 0.01],
        ...
        self.model = ManipulationTask(
            ...
            mujoco_objects=(self.slide, self.door)
        )
    ...
    def _setup_references(self):
        ...
        self.object_body_ids["slide"] = self.sim.model.body_name2id(self.slide.slide_body)
        self.slide_handle_site_id = self.sim.model.site_name2id(self.slide.important_sites["handle"])
        self.slide_hinge_qpos_addr = self.sim.model.get_joint_qpos_addr(self.slide.joints[0])
        ...
        self.door_hinge_qpos_addr = self.sim.model.get_joint_qpos_addr(self.door.joints[0])
    ...
    def _setup_observables(self):
        ...
        if self.use_object_obs:
            ...
            def slide_pos(obs_cache):
                return np.array(self.sim.data.body_xpos[self.object_body_ids["slide"]])
            ...
            @sensor(modality=modality)
            def slide_to_eef_pos(obs_cache):
                return (
                    obs_cache["slide_pos"] - obs_cache[f"{pf}eef_pos"]
                    if "slide_pos" in obs_cache and f"{pf}eef_pos" in obs_cache
                    else np.zeros(3)
                )
            ...
            @sensor(modality=modality)
            def door_handle_pos(obs_cache):
                return self._door_handle_xpos
            ...
            @sensor(modality=modality)
            def door_handle_to_eef_pos(obs_cache):
                return (
                    obs_cache["door_handle_pos"] - obs_cache[f"{pf}eef_pos"]
                    if "door_handle_pos" in obs_cache and f"{pf}eef_pos" in obs_cache
                    else np.zeros(3)
                )

            @sensor(modality=modality)
            def door_hinge_qpos(obs_cache):
                return np.array([self.sim.data.qpos[self.door_hinge_qpos_addr]])
    ...
    def _reset_internal(self):
        ...
        self.handle_current_progress = self.sim.data.qpos[self.slide_hinge_qpos_addr]
        ...
        self.handle_current_progress = self.sim.data.qpos[self.door_hinge_qpos_addr]
    ...
    def visualize(self, vis_settings):
        ...
        Args:
            vis_settings (dict): Visualization keywords mapped to T/F, determining whether that specific
                ...
                options specified.
        """
        #TODO haven't modified this, will depend on how stuff is used
        ...
        if vis_settings["grippers"]:
            self._visualize_gripper_to_target(
                gripper=self.robots[0].gripper, target=self.slide.important_sites["handle"], target_type="site"
            )
    ...
    def _slide_handle_xpos(self):
        ...
        """
        return self.sim.data.site_xpos[self.slide_handle_site_id]
    ...
    @property
    def _gripper_to_door_handle(self):
        """
        Calculates distance from the gripper to the door handle.
        Returns:
            np.array: (x,y,z) distance between handle and eef
        """
        return self._door_handle_xpos - self._eef_xpos